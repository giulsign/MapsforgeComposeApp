...
/**
 * 🔧 FIXED VERSION - Singleton Pattern
 * Questo servizio ora mantiene la connessione Firebase anche quando cambi schermata
 */
class FirebaseLocationService private constructor(private val context: Context) {
    ...
    companion object {
         ...
         * Deve essere inizializzato una sola volta (es. in Application o MainActivity)
         */
        ...
        /**
         * Verifica se il singleton è stato inizializzato
         */
    ...
    private fun getOrCreateDeviceId(): String {
        ...
        if (deviceId == null) {
            deviceId = UUID.randomUUID().toString()
            prefs.edit().putString(KEY_DEVICE_ID, deviceId).apply()
            Log.d(TAG, "🆕 New device ID generated: ${deviceId.take(8)}...")
        }
    ...
    suspend fun updateLocation(location: Location): FirebaseResult<Boolean> {
        ...
        return try {
            ...
            Log.d(TAG, "📍 Location updated: ${location.latitude}, ${location.longitude} (role: $currentRole)")
            FirebaseResult.Success(true)
