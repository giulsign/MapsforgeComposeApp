...
class BluetoothGroupSharing(
    ...
    data class DiscoveredGroup(
        ...
        val rssi: Int,
        val bluetoothDevice: String // Address for device BLE
    )
    ...
    private fun setupGattServer(group: Group) {
        if (!hasBluetoothPermissions()) {
            Log.e(TAG, "Cannot setup GATT server without permissions.")
            return
        }
        ...
        try {
            ...
            Log.e(TAG, "Exception setting up GATT server", e)
            _sharingState.value = SharingState.Error("GATT setup failed: ${e.message}")
        }
    }

    // ==================== CALLBACKS ====================

    // Add this callback definition alongside your other callbacks
    private val gattServerCallback = object : BluetoothGattServerCallback() {
        override fun onConnectionStateChange(device: BluetoothDevice?, status: Int, newState: Int) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                // If the process wasn't successful, show an error
                if (_sharingState.value !is SharingState.Success) {
                    _sharingState.value = SharingState.Error("Client disconnected")
                }
            }
        }
        ...
        override fun onCharacteristicReadRequest(
            ...
            if (characteristic!!.uuid == CHARACTERISTIC_UUID) {
                ...
                try {
                    // Respond with some value, e.g., the public key for the initial handshake
                    // This logic might need to be adjusted based on your exact protocol.
                    ...
                    val message = HandshakeMessage(
                        ...
                        groupName = "GROUP_NAME_PLACEHOLDER" // Replace with actual group name if available
                    )
                ...
                // Invalid characteristic requested
                try {
        ...
        override fun onNotificationSent(device: BluetoothDevice?, status: Int) {
            ...
            Log.d(TAG, "Notification sent with status: $status")
        }
    }
    ...
    private val scanCallback = object : ScanCallback() {
        override fun onScanResult(callbackType: Int, result: ScanResult?) {
            result?.let { scanResult ->
                try {
                    val serviceData =
                        scanResult.scanRecord?.getServiceData(ParcelUuid(SERVICE_UUID))
                    val groupName =
                        serviceData?.let { String(it, Charsets.UTF_8) } ?: "Unknown Group"
                    ...
                    if (currentList.none { it.bluetoothDevice == discovered.bluetoothDevice }) {
                        ...
                        Log.d(TAG, "Group already discovered: $groupName from $deviceName")
                    }
                } catch (e: SecurityException) {
